...
class Boid:
    ...
    def new_velocity(self, curr_iteration, swarms_best_location):
        # new personal velocity = old personal velocity + personal influence + social influence
        c1 = 1
        c2 = 3
        inertia = 0.5
        self.velocity = np.multiply(inertia, self.velocity) + c1 * np.random.random() * (
            np.subtract(self.best_position, self.position)) + c2 * np.random.random() * (
                            np.subtract(swarms_best_location, self.position))
...
class Optimizer:
    ...
    def animate_schwefel(self, title):
        # Mesh points
        x = np.arange(-self.axis_limit, self.axis_limit + 1, 1)
        y = np.arange(-self.axis_limit, self.axis_limit + 1, 1)
        surface_x, surface_y = np.meshgrid(x, y)
        surface_z = self.cost_function([surface_x, surface_y])
        fig = go.Figure(data=[go.Surface(x=x, y=y, z=surface_z, opacity=0.5, colorscale="blues")])

        points_to_draw = []

        for _, swarm_name in enumerate(self.swarms):
            for i, _ in enumerate(self.swarms[swarm_name].xs):
                point = [self.swarms[swarm_name].xs[i][0], self.swarms[swarm_name].ys[i][0],
                         self.swarms[swarm_name].zs[i][0]]
                points_to_draw.append(point)

        print(points_to_draw)
        points_to_draw = np.column_stack(points_to_draw)

        print(points_to_draw)
        fig.add_trace(
            go.Scatter3d(x=points_to_draw[0], y=points_to_draw[1], z=points_to_draw[2], mode="markers", name="points"))
        fig.update_traces(marker=dict(size=4), selector=({'name': 'points'}))
...
'''
    ...
    def animate_schwefel(self, title, filepath):
        colors = ['r', 'g', 'b', 'c', 'm']
        ...
        anim = ani.FuncAnimation(fig=figure, func=update_schwefel, frames=self.iterations, interval=250, repeat=False)
        video_writer = ani.FFMpegWriter(fps=10)
        anim.save(filename=filepath, writer=video_writer)
        plt.close()
'''
'''
...
'''
    ...
    def animate_banana(self, title, filepath):
        colors = ['r', 'g', 'b', 'c', 'm']
        ...
        anim = ani.FuncAnimation(fig=figure, func=update_banana, frames=self.iterations, interval=250, repeat=False)
        video_writer = ani.FFMpegWriter(fps=10)
        anim.save(filename=filepath, writer=video_writer)
        plt.close()
'''
...
if __name__ == "__main__":
    ...
    # Create the animations and save the video files
    '''
    
    pso_banana.animate_banana(title="Banana cost function",
                              filepath="C:/Users/roope/Kurssitiedostot/Artificial intelligence/HA/PSO_banana_optimization_final_2025_2.mp4")
    pso_schwefel.animate_schwefel(title="Schwefel cost function",
                                  filepath="C:/Users/roope/Kurssitiedostot/Artificial intelligence/HA/PSO_schwefel_optimization_final_2025_2.mp4")
    '''
